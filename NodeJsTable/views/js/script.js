...
class nactiJSONTabulku{

    constructor(){

        const nJSON = new nactiJSONScales();
        this.nactiDataUrl(nJSON, "http://localhost:3000/table/data");
        this.nactiDataUrl(nJSON, "http://localhost:3000/table/idColSubtable");
        this.nactiDataUrl(nJSON, "http://localhost:3000/table/idsColumns");
...
class nactiJSONScales{
    ...
    //obcas se muzou data z webove sluzby nacist v jinem poradi
    //pak je treba data prohodit zde
    prohodSpravnePoradi(jsonPole){

        var nazvyKlicuPoradi = [];
        nazvyKlicuPoradi.push('row');
        nazvyKlicuPoradi.push('variable');
        nazvyKlicuPoradi.push('colId');
        

        var polozkyVeSpravnemPoradi = []

        for (let i = 0; i < nazvyKlicuPoradi.length; i++) {
            var nazevKlice = nazvyKlicuPoradi[i];
            var polozka = this.vratPolozkuDleKlice(jsonPole, nazevKlice);

            polozkyVeSpravnemPoradi.push(polozka);
        }    

        return(polozkyVeSpravnemPoradi);

    }


    vratPolozkuDleKlice(jsonPole, nazevKlice){

        var polozka = false;

        for (let i = 0; i < jsonPole.length; i++) {
            var data = jsonPole[i];
            var shoda = this.detekujZdaSeJednaOKlic0(data, nazevKlice);
    ...
    detekujZdaSeJednaOKlic0(data, nazevKlice){

        var polozka0 = data[0];
        var nazvyKlicu = Object.keys(polozka0);

        var shoda = false

        if(nazvyKlicu[0] == nazevKlice){
            shoda = true
        }
...
class vytvorTabulku{
    ...
    vratAppendStrHlavicky(obj, objVar, idExp){
        ...
        var poleRadkuHlavicky = this.vratPoleRadkuHlavicky(objVar, idExp);
    ...
    vratPoleRadkuHlavicky(objVar, idExp){
    ...
    vlozSubTabulku(sloucenySloupec, nazevSloupce, id){
        ...
        subTabulka =    '<table>\n' +
                        '   <tr>\n' +
                        '       <th><button class="buttLeft" id=' + idDoleva + '>&lt;&lt;</button></th>\n' +
                        '       <th>' + nazevSloupce + '</th>\n' +
                        '       <th><button class="buttRight" id=' + idDoprava + '>&gt;&gt;</button></th>\n' +
                        '   </tr>\n'+
                        '</table>';
...
$( 'document' ).ready(function(){
    var tabNew = new nactiJSONTabulku();
});
